...
async function updateOrganizationInfo(req: Request, res: Response) {
  try {
    ...
    if (
      Object.hasOwn(updateObj, "collectionsDisplayLabel") &&
      isEmptyString(updateObj.collectionsDisplayLabel ?? "")
    ) {
...
async function updateBrandingImageAsset(req: Request, res: Response) {
  try {
    ...
    const updateRes = await Organization.updateOne(
      { orgID },
      {
        [assetName]: assetURL,
      }
...
function validate(method: string) {
  switch (method) {
    case "getinfo":
      return [
        param("orgID", conductorErrors.err1)
          .exists()
    ...
    case "updateinfo":
      return [
        param("orgID", conductorErrors.err1)
          .exists()
          .isLength({ min: 2, max: 50 }),
        body("aboutLink", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isURL(),
        body("commonsHeader", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ max: 200 }),
        body("commonsMessage", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ max: 500 }),
        body("collectionsDisplayLabel", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ max: 200 }),
        body("collectionsMessage", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ max: 500 }),
        body("primaryColor", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ min: 7, max: 7 })
          .isHexColor(),
        body("footerColor", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isLength({ min: 7, max: 7 })
          .isHexColor(),
        body("addToLibreGridList", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isBoolean()
          .toBoolean(),
        body("catalogMatchingTags", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isArray()
          .customSanitizer(ensureUniqueStringArray),
        body("supportTicketNotifiers", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isArray()
          .isEmail(),
        body("defaultAssetTagFrameworkUUID", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isUUID(),
        body("customOrgList", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isArray()
          .customSanitizer(ensureUniqueStringArray),
        body("commonsModules", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isObject()
          .custom(validateCommonsModules),
        body("showCollections", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isBoolean()
          .toBoolean(),
        body("assetFilterExclusions", conductorErrors.err1)
          .optional({ checkFalsy: true })
          .isArray()
          .customSanitizer(ensureUniqueStringArray),
      ];
    case "updateBrandingImageAsset":
      return [
        ...
        param("assetName", conductorErrors.err1)
          .exists()
          .isString()
          .custom(validateBrandingAssetName),
      ];
  }
}
