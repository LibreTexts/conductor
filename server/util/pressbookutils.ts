...
export class PressBookScraper {
  ...
  private async upsertCXOnePage(
    ...
    const res = await CXOneFetch({
      ...
      options: {
        method: "POST",
        body: `${isContainer ? MindTouch.Templates.POST_CreateBookChapter : MindTouch.Templates.POST_CreateBookTopic}\n${contentHtml || ""}`,
    ...
    if (isContainer) {
      ...
      await addPageProperty(this.subdomain, pageID, "GuideDisplay", "single", "PUT");
