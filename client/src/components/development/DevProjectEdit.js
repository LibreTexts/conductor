...
class DevProjectEdit extends Component {
    ...
    saveChanges() {
        ...
        if (newProject.resourceURL !== this.state.previous.resourceURL) {
            toSend.resourceURL = newProject.resourceURL;
        }
        axios.post('/development/projects/update', toSend, {
            headers: {
            ...
            if (!res.data.err) {
                if (res.data.id) {
                    this.props.history.push(`/development/projects/${res.data.id}?showUpdateSuccess=true`);
                }
            } else {
    ...
    cancelChanges() {
        this.props.history.push(`/development/projects/${this.state.id}`);
    }
        ...
        return(
            <Grid className='component-container' divided='vertically'>
                ...
                <Grid.Row>
                    <Grid.Column width={16}>
                        <Segment>
                            <Link to={`/development/projects/${this.state.id}`}>
