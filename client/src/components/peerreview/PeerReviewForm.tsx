...
interface PeerReviewFormProps {
  /** The project the review is submitted against. Drives rubric load + submit target. */
  projectID: string;
  /** Resource title shown beneath the heading (book or project title). */
  resourceTitle?: string;
  /** When true (and the user is not authenticated), collect name/email. */
  showAnonFields?: boolean;
  /** Adjusts the privacy notice wording for public-facing (Commons) contexts. */
  isPublicView?: boolean;
  /** Cancel + header button target. */
  onCancel: () => void;
...
/**
 * Shared peer review submission form. Loads the project's rubric, renders the
 * dynamic prompts, validates with React Hook Form field rules, and POSTs the
 * review. Used by both the
 * Commons book flow (`/book/:bookID/submit-peer-review`) and the authenticated
 * project flow (`/projects/:id/submit-peer-review`).
 */
const PeerReviewForm: React.FC<PeerReviewFormProps> = ({
  projectID,
  resourceTitle,
  showAnonFields = false,
  isPublicView = false,
  onCancel,
  onSuccess,
}) => {
  const user = useTypedSelector((state) => state.user);
  const collectAnon = showAnonFields && !user.isAuthenticated;
  ...
  const submitMutation = useSubmitPeerReview({ onSuccess });
