...
import { usePeerReviewRubric } from "../../hooks/usePeerReviewRubric";
import { useSubmitPeerReview } from "../../hooks/useSubmitPeerReview";
import type { PeerReviewFormValues } from "./peerReviewFormSchema";
...
interface PeerReviewFormProps {
  /** The Project the review is attached to — the rubric and submission are always scoped to this. */
  projectID: string;
  /** Displayed under the heading, e.g. the linked textbook's title or the project's title. */
  resourceTitle?: string;
  /** Softens the "visible to all" notice to match anonymous/public-facing submission flows. */
  isPublicView?: boolean;
  /** Where to navigate on Cancel or after a successful submission. */
  redirectPath: string;
  onCancel: () => void;
}

/**
 * Rubric-driven peer review submission form, shared by every entry point
 * (Commons book page, Conductor project page). Scoped entirely by
 * `projectID` — callers that only know a bookID must resolve it to a
 * projectID first (see Commons' book detail lookup).
 */
const PeerReviewForm: React.FC<PeerReviewFormProps> = ({
  ...
  const submitMutation = useSubmitPeerReview(redirectPath);
