            ...
            >
              {shippingOptions.map((deliveryMethod) => {
                ...
                minDeliveryDate.setDate(
                  minDeliveryDate.getDate() + deliveryMethod.total_days_min
                );
                const maxDeliveryDate = new Date();
                maxDeliveryDate.setDate(
                  maxDeliveryDate.getDate() + deliveryMethod.total_days_max
                );

                const minDeliverDateFormatted = new Intl.DateTimeFormat(
                  "en-US",
                  {
                    month: "numeric",
                    day: "numeric",
                ...
                ).format(minDeliveryDate);

                const maxDeliverDateFormatted = new Intl.DateTimeFormat(
                  "en-US",
                  {
                    month: "numeric",
                    day: "numeric",
                    ...
                    <span className="flex flex-1">
                      <span className="flex flex-col">
                        ...
                        <span className="mt-1 flex items-center text-sm text-gray-500">
                          Est. Delivery {minDeliverDateFormatted} -{" "}
                          {maxDeliverDateFormatted}
                        </span>
