...
type SelectedCatalogBookFilters = {
  library?: string;
  subject?: string;
  location?: string[];
  license?: string;
...
const CatalogBookFilters = () => {
  ...
  const [selectedFilters, setSelectedFilters] =
    useState<SelectedCatalogBookFilters>({});
  ...
  async function getLicenseOptions() {
    try {
      ...
      const newLicenseOptions = [{ key: "empty", text: "Clear...", value: "" }];
            ...
            <Dropdown.Menu className={MENU_CLASSES}>
              {cidOptions.map((cid) => (
                <Dropdown.Item
                  key={cid.key}
                  onClick={() =>
                    setSelectedFilters({ ...selectedFilters, cid: cid.value })
                  }
                >
                  {cid.text}
                </Dropdown.Item>
