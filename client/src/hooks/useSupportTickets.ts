...
const useSupportTickets = (queue: string, params: UseSupportTicketsParams) => {
  ...
  const queryObj = useQuery<OpenSupportTicketsResponse>({
    queryKey: queryKey,
    queryFn: async () => {
      const res = await axios.get<OpenSupportTicketsResponse>(
        "/support/ticket/open",
        {
          params: {
            queue,
            ...(params.query &&
              params.query?.length > 3 && { query: params.query }),
            page: params.page,
            limit: params.items,
            sort: params.sort,
  ...
  return { ...queryObj, totalPages, invalidate, queryKey };
};
